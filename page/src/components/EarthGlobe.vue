          ...
          polygon.forEach((ring, ringIndex) => {
            if (ringIndex === 0 && ring.length > 3) {
              const points = ring.map((coord) =>
                latLngToVector3(coord[1], coord[0], config.countries.radius),
              );
...
const initThree = async () => {
  ...
  renderer.setClearColor(
    ...
    config.renderer.backgroundAlpha,
  );
...
const onMouseMove = (event) => {
  ...
  earth.rotation.x = Math.max(
    -config.globe.maxVerticalRotation,
    Math.min(config.globe.maxVerticalRotation, earth.rotation.x),
  );
