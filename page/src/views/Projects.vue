...
const completedProjects = computed(
  () =>
    projects(t.value).filter((p) => p.status === t.value.status.completed)
      .length,
);
const inProgressProjects = computed(
  () =>
    projects(t.value).filter((p) => p.status === t.value.status.inProgress)
      .length,
);
...
onMounted(() => {
  ...
  script.type = "module";
  script.src = "https://unpkg.com/@splinetool/viewer@1.12.67/build/spline-viewer.js";
    ...
    <section class="relative">
      <div class="m-auto text-center">
        ...
        <div class="relative h-88">
          <spline-viewer
            class="m-auto"
            url="https://prod.spline.design/tuWb-1s-7gHqANF7/scene.splinecode"
          ></spline-viewer>
          <div
            class="absolute inset-x-0 bottom-0 h-full from-black/40 to-transparent pointer-events-none"
          ></div>
        </div>

        <div class="flex flex-wrap justify-center gap-8 pt-4 pb-8">
          <div
            class="bg-white/10 backdrop-blur-sm rounded-lg px-6 py-4 border border-white/20"
          >
    ...
    <section id="projects">
      <div class="from-black/40 to-transparent m-auto">
        <div class="relative mb-8">
          ...
          />
          <svg
            class="absolute right-7 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div class="relative flex flex-wrap justify-center gap-2">
          <button
            v-for="category in categories(t)"
            :key="category"
